...
class LegacyItemDetailsSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        ...
        if representation.get("last_price") is not None:
            representation["last_price"] = float(
                Decimal(str(instance.last_price)).quantize(Decimal("0.01"))
            )

        if representation.get("average_price") is not None:
            representation["average_price"] = float(
                Decimal(str(instance.average_price)).quantize(Decimal("0.01"))
            )
