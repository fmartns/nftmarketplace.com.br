...
def verify_webhook_signature(raw_body: bytes, signature_from_header: str) -> bool:
    ...
    if not ABACATEPAY_PUBLIC_KEY:
        logger.error("ABACATEPAY_PUBLIC_KEY não configurada. Configure a variável de ambiente ABACATEPAY_PUBLIC_KEY")
        return False
...
def AbacatePayWebhookView(request):
    ...
    try:
        ...
        logger.info(f"Webhook recebido: event={event_type}, id={log_id}, devMode={dev_mode}")
        ...
        if event_type == "billing.paid":
            ...
            logger.info(f"Processando billing.paid - PIX ID: {pix_id}, Amount: {payment_amount_cents}")
            ...
            if payment_amount_cents > 0:
                ...
                if pending_billings.count() == 1:
                    ...
                    logger.info(f"Encontrada cobrança única: {billing_found.billing_id}")
                    ...
                    if not list_response.get("error"):
                        ...
                        for api_billing in api_billings:
                            ...
                            if api_status == "PAID" and api_amount == payment_amount_cents:
                                try:
                                    billing_found = AbacatePayBilling.objects.get(billing_id=api_billing_id)
                                    logger.info(f"Encontrada cobrança paga na API: {billing_found.billing_id}")
                                    break
                    ...
                    if not billing_found:
                        billing_found = pending_billings.first()
                        logger.info(f"Usando cobrança mais recente: {billing_found.billing_id}")
            ...
            if not billing_found:
                ...
                return JsonResponse({"status": "ok", "message": "Billing not found but webhook processed"})
            ...
            if not payment_obj:
                payment_amount = Decimal(payment.get("amount", 0)) / 100 if payment else billing.amount
                ...
                payment_obj.payment_method = payment.get("method", "PIX") if payment else payment_obj.payment_method or "PIX"
                payment_obj.save()
