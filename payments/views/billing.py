...
class BillingCreateView(APIView):
    ...
    def post(self, request):
        ...
        metadata.update({
            ...
            "user_name": f"{request.user.first_name or ''} {request.user.last_name or ''}".strip() or request.user.username or "",
        })
        
        if not customer.external_id:
            import re
            cpf_clean = re.sub(r'\D', '', request.user.cpf or '')
            has_valid_cpf = cpf_clean and cpf_clean != "00000000000" and len(cpf_clean) == 11
            ...
            if has_valid_cpf:
                ...
                if not user_name:
                    ...
                    if not user_name:
                        logger.warning(f"Usuário {request.user.id} não possui nome completo, pulando criação de cliente")
                    else:
                        ...
                        if not customer_response.get("error") and customer_response.get("data"):
                            customer.external_id = customer_response["data"].get("id", "")
                            customer.metadata = customer_response["data"].get("metadata", {})
                            customer.save()
                else:
                    customer_response = AbacatePayService.create_customer(
                        user_id=str(request.user.id),
        ...
        for item in order.items.all():
            ...
            elif hasattr(item.item, 'description_pt_br') and item.item.description_pt_br:
                item_description = item.item.description_pt_br
            
            products.append({
                "externalId": external_id,
                "name": item_name,
                "description": item_description or f"{item_name} - Quantidade: {item.quantity}",
        ...
        if billing_response.get("error"):
            ...
            if isinstance(error_details, dict) and error_details.get("statusCode") == 401:
                return Response(
                    {
                        "error": "API key não configurada",
                        "message": error_details.get("message", "ABACATEPAY_API_KEY não está configurada"),
                        "details": "Configure a variável de ambiente ABACATEPAY_API_KEY no arquivo .env",
                    },
                    status=status.HTTP_503_SERVICE_UNAVAILABLE,
                )
            
            if isinstance(error_details, dict) and error_details.get("statusCode") == 404:
                docs_url = getattr(settings, "ABACATEPAY_DOCS_URL", "https://docs.abacatepay.com")
                return Response(
                    {
            ...
            return Response(
                {"error": "Erro ao criar cobrança na AbacatePay", "details": error_details},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
...
class BillingListView(APIView):
    ...
    def get(self, request):
        ...
        billings = AbacatePayBilling.objects.filter(
...
class BillingPixQRCodeView(APIView):
    ...
    def post(self, request, billing_id):
        ...
        if qrcode_response.get("error"):
            return Response(
                {"error": "Erro ao criar QRCode PIX", "details": qrcode_response["error"]},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
...
class BillingPixCheckView(APIView):
    ...
    def get(self, request, billing_id):
        ...
        if check_response.get("error"):
            return Response(
                {"error": "Erro ao verificar status PIX", "details": check_response["error"]},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
...
class BillingSimulateView(APIView):
    ...
    def post(self, request, billing_id):
        ...
        if simulate_response.get("error"):
            return Response(
                {"error": "Erro ao simular pagamento", "details": simulate_response["error"]},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
